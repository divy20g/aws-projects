...
s3_object_name = 'source-file/df_from_rds.parquet'
...
def lambda_handler(event,context):
    ...
    df.head() 
    #print (df.head())
    #wr.s3.to_parquet(df,'s3://database-query-results-stage/newfile.parquet')

    with io.StringIO() as df_buffer:
        df.to_parquet(df_buffer, index=False)

        response = s3_client.put_object(
            Bucket=bucket_name, Key=s3_object_name, Body=df_buffer.getvalue()
        )
